...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        """
        # 添加这段代码，根据类别名称设置不同颜色 (BGR格式)
        if label:
            # 提取类别名称（去掉置信度部分）
            class_name = label.split(' ')[0]
            
            # 根据类别名称设置颜色 (BGR格式)
            if 'Igneous_Basalt' in class_name:
                color = (0, 0, 255)  # 红色 (BGR)
            elif 'Sedimentary_Limestone' in class_name:
                color = (22, 205, 145)  # 绿色 (BGR)
            elif 'Igneous_Granite' in class_name:
                color = (34, 114, 255)  # 橙色 (BGR)
            # 其他类别保持原有颜色不变
